...
/**
 * ZEPRS implementation of BeanProcessor to handle null values
 * Created by IntelliJ IDEA.
 * User: ckelley
 * Date: Jun 5, 2005
 * Time: 10:14:22 PM
 */
public class ZEPRSBeanProcessor extends BeanProcessor {

	/**
	 * Commons Logging instance.
	 */
	private static Log log = LogFactory.getFactory().getInstance(ZEPRSBeanProcessor.class);
     ...
     * <p/>
     * <p/>
     * This implementation first cals getObject on some of the numeric data types to check for null.
     * If not null, calls the appropriate <code>ResultSet</code> getter
    ...
    protected Object processColumn(ResultSet rs, int index, Class propType)
        ...
        if (propType.equals(String.class)) {
            ...
            return value;
                ...
                propType.equals(java.sql.Date.class)) {
            Object value = null;
            try {
				value = rs.getObject(index);
			} catch (SQLException e) {
				// Improperly-formated value
				value = null;
				log.debug("Improperly-formatted Date value at index: " + index + ". Error: " + e);
				e.printStackTrace();
			}
            if (value != null) {
                value = new java.sql.Date(rs.getDate(index).getTime());
            }
            return value;

        }  else if (
                propType.equals(Time.class)) {
            Object value = null;
            value = rs.getObject(index);
            if (value != null) {
                value = new Time(rs.getTime(index).getTime());
            }
            return value;
