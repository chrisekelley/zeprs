...
import org.cidrz.webapp.dynasite.valueobject.User;
 ...
 * Time: 10:14:22 PM
 */
public class AuditInfoBeanProcessor extends BeanProcessor {
        ...
        for (int col = 1; col <= cols; col++) {
            ...
            for (int i = 0; i < props.length; i++) {
                ...
                if (columnName.equalsIgnoreCase(propName)) {
                    ...
                    break;
                } else if (clazz.toString().equals("class org.cidrz.webapp.dynasite.valueobject.AuditInfo")) {
                    columnToProperty[col] = i;
                }
            }
        }
    ...
    protected Object processColumn(ResultSet rs, int index, Class propType)
        ...
        } else if (propType.equals(AuditInfo.class)) {
            if (rs.getMetaData().getColumnTypeName(index).equals("DATETIME")) {
                //  rs.getMetaData().getColumnName(index);
                AuditInfo ai = new AuditInfo();
                ai.setLastModified(rs.getTimestamp(index));
                return ai;
            } else if (rs.getMetaData().getColumnTypeName(index).equals("VARCHAR")) {
                //  rs.getMetaData().getColumnName(index);
                AuditInfo ai = new AuditInfo();
                User user = new User();
                if (rs.getMetaData().getColumnName(index).equals("auditInfo.lastModifiedBy.firstName")) {
                    user.setFirstName(rs.getString(index));
                    ai.setLastModifiedBy(user);
                } else if (rs.getMetaData().getColumnName(index).equals("auditInfo.lastModifiedBy.lastName")) {
                ...
                /*try {
                    BeanUtils.copyProperty(ai,"rs.getMetaData().getColumnName(index)", rs.getString(index));
    ...
    private void callSetter(Object target, PropertyDescriptor prop, Object value)
        ...
        if (value != null) {
            if (value.getClass().toString().equals("class org.cidrz.webapp.dynasite.valueobject.AuditInfo")) {
                Auditable info = (Auditable) target;
                AuditInfo infoValue = (AuditInfo) value;
                User user = info.getAuditInfo().getLastModifiedBy();
                User userValue = infoValue.getLastModifiedBy();
                if (user == null) {
                    user = new User();
                }
                if (info.getAuditInfo() != null) {
                    try {
                        if (userValue != null) {
                            if (userValue.getFirstName() != null) {
                                user.setFirstName(userValue.getFirstName());
                            } else if (userValue.getLastName() != null) {
                                user.setLastName(userValue.getLastName());
                            }
                            info.getAuditInfo().setLastModifiedBy(user);
                        } else if (infoValue.getLastModified() != null) {
                            info.getAuditInfo().setLastModified(infoValue.getLastModified());
                        }

                        // BeanUtils.copyProperties(info.getAuditInfo(), infoValue);
