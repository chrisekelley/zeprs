...
images[0] = new Image();
images[1] = new Image();
images[0].src = "/zeprs/images/plus.gif";
...
function calcTime(fieldId, widgetName){
    if (widgetName == null) {
		widgetName = "field";
	}
    var hourField = document.getElementById("hour" + fieldId);
   // var hourValue = hourField[hourField.selectedIndex].value
    var hourValue = hourField.value;
    var minuteField = document.getElementById("minute" + fieldId);
    //var minuteValue = minuteField[minuteField.selectedIndex].value
    var minuteValue = minuteField.value;
    var timeField = document.getElementById(widgetName + fieldId);
    if ((hourValue == '' )) {
        alert("Please select a value for hours.")
        var timeValue = hourField.value + ":" + minuteField.value + ":00"
        timeField.value = timeValue;
    } else if ((hourValue = '' ) && (minuteValue = '' )) {
        alert("Please select a value for hours and minutes.")
        var timeValue = hourField.value + ":" + minuteField.value + ":00"
        timeField.value = timeValue;
    } else {
        var timeValue = hourField.value + ":" + minuteField.value + ":00"
        timeField.value = timeValue;
		//alert("timeValue: " + timeValue + " hourValue: " + hourField.value + " minuteValue: " + minuteValue);
    }
}

function setTimeField(fieldId, widgetName)
{
    if (widgetName == null) {
		widgetName = "field";
	}
    ...
    document.getElementById(widgetName + fieldId).value = clock;
}
