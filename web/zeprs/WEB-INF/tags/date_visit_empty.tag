...
<span onclick="showCalendar('${yearnow}','${monthnow}','${datenow}','dd/MM/yyyy','form${encounterForm.id}','field${field.id}',event,${lastTwentyYears},${twoYears});" class="underline">Date:</span>
<span id="spanfield${field.id}" onclick="showCalendar('${yearnow}','${monthnow}','${datenow}','dd/MM/yyyy','form${encounterForm.id}','field${field.id}',event,${lastTwentyYears},${twoYears});"></span>
<div id="slcalcodfield${field.id}" style="position:absolute; left:10px; top:100px; z-index:10; visibility:hidden;">
<script type="text/javascript">printCalendar("Sun","Mon","Tue","Wed","Thu","Fri","Sat","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","${datenow}","${monthnow}","${yearnow}","field${field.id}","","${theDiv}");</script></div>
<span id="correctDateVisit${pos}" style="display:none;">
    <input type="text" id="field${field.id}" class="disabled-date" size="9" onfocus="this.blur()" name="field${field.id}"/>
</span>