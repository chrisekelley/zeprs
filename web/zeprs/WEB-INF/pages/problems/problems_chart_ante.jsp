...
<logic:present name="zeprs_session" property="pregnancy">
    <c:if test="${empty zeprs_session.pregnancy.datePregnancyEnd}"> | <a href='<c:out value="/zeprs/${probUrl}"/>'>Create Problem</a></h2></c:if>
</logic:present>
<div id="anteProbList">
    ...
    <c:choose>
        ...
        <c:when test="${thisproblem.class.name=='org.zeprs.rules.impl.EncounterOutcome'}">
            <c:choose>
                ...
                <tr class="currentProblem">
                    <td><a href='<c:out value="/zeprs/${editUrl}"/>'>Form Request: ${thisproblem.form.label}</a></td>
                    <td width="50px" ><fmt:formatDate type="both" pattern="dd MMM yy" value="${thisproblem.auditInfo.created}" /></td>
                </tr>
                </c:when>
                <c:otherwise>
